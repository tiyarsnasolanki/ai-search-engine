...
const Navbar2 = ({ onSearch }) => {
  ...
  const handleLogout = () => {
    ...
    setUser(null);
    setShowLogout(false); // Reset logout visibility
    navigate("/login");
  };
  ...
  const handleSearch = (e) => {
    ...
    onSearch(query); // Assuming you have a parent function to handle the search
  };

  const navItems = [
    { name: "Home", path: "/" },
    // You can add other nav items here as needed
  ];
  ...
  return (
    <nav className="navbar h-20 px-6 lg:px-12 bg-gradient-to-r from-blue-500 to-purple-600 text-white flex items-center justify-between shadow-lg">
      ...
      {/* Center section with search bar (re-enabled) */}
